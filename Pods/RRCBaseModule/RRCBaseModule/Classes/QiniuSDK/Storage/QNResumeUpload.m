...
- (void)nextTask:(UInt32)offset retriedTimes:(int)retried host:(NSString *)host {
    ...
    if (offset == self.size) {
        QNCompleteBlock completionHandler = ^(QNResponseInfo *info, NSDictionary *resp) {
            ...
            } else if (info.couldRetry && retried < self->_config.retryMax) {
    ...
    QNCompleteBlock completionHandler = ^(QNResponseInfo *info, NSDictionary *resp) {
        if (info.error != nil) {
            ...
            if (retried >= self->_config.retryMax || !info.couldRetry) {
                self.complete(info, self.key, resp);
                return;
            }
        ...
        if (ctx == nil || crc == nil || [crc unsignedLongValue] != self->_chunkCrc) {
            [self nextTask:offset retriedTimes:retried host:host];
            return;
        }
        self->_contexts[offset / kQNBlockSize] = ctx;
