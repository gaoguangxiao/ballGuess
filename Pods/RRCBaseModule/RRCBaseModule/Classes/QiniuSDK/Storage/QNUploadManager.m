    ...
    [_config.zone preQuery:t on:^(int code) {
        ...
        QNFormUpload *up = [[QNFormUpload alloc]
            ...
            withCompletionHandler:complete
                       withOption:option
                            withHttpManager:self->_httpManager
                            withConfiguration:self->_config];
        QNAsyncRun(^{
            [up put];
        });
    }];
...
- (void)putFileInternal:(id<QNFileDelegate>)file
    ...
    @autoreleasepool {
        ...
        [_config.zone preQuery:t on:^(int code) {
            ...
            if ([file size] <= self->_config.putThreshold) {
            ...
            if (self->_config.recorder != nil && self->_config.recorderKeyGen != nil) {
                recorderKey = self->_config.recorderKeyGen(key, [file path]);
            }

            NSLog(@"recorder %@", self->_config.recorder);
            ...
            QNResumeUpload *up = [[QNResumeUpload alloc]
                ...
                withCompletionHandler:complete
                           withOption:option
                                  withRecorder:self->_config.recorder
