...
-(void)setZoomAnimationScale{
    CGFloat ScreenWidth = self.layout.itemSize.width;
    //保证visibleCells 有数值
    [self.collectionView layoutIfNeeded];
    for (UICollectionViewCell * cell in self.collectionView.visibleCells) {
        CGPoint centerPoint = [self convertPoint:cell.center fromView:self.collectionView];
        CGFloat d = fabs(centerPoint.x - ScreenWidth*0.5)/cell.contentView.bounds.size.width;
        CGFloat scale = 1-0.1*d ;
        cell.layer.transform = CATransform3DMakeScale(1.0, scale, 1);
    }
}
...
- (CGFloat)caculateOffsetXAtIndexSection:(TYIndexSection)indexSection{
    ...
    CGFloat leftEdge = _isItemCenter ? edge.left : edge.left + edge.right;
