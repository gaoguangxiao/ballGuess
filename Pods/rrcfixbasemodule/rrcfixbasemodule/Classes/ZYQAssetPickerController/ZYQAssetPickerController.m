        ...
        if ([_originAssetGroup isKindOfClass:[ALAssetsGroup class]]) {
            ...
            [[PHImageManager defaultManager] requestImageForAsset:tmpAsset targetSize:CGSizeMake(200, 200) contentMode:PHImageContentModeAspectFill options:requestOptions resultHandler:^(UIImage *result, NSDictionary *info){
                ...
                if (downloadFinined) {
                    ...
                    self->_cacheThumbnail=[UIImage imageWithCGImage:result.CGImage scale:scale orientation:UIImageOrientationUp];
                    
                    self->_getThumbnail(_cacheThumbnail);
...
-(void)setGetVideo:(void (^)(NSURL *))getVideo fromNetwokProgressHandler:(void (^)(double ,NSError * , BOOL *, NSDictionary *))progressHandler{
    _getVideo=getVideo;
    
    if (_originAsset) {
        if (_cacheVideoUrl) {
            _getVideo(_cacheVideoUrl);
            return;
        }
        
        if ([_originAsset isKindOfClass:[ALAsset class]]) {
            _cacheVideoUrl=[[(ALAsset*)_originAsset defaultRepresentation] url];
            _getVideo(_cacheVideoUrl);
        }
        else if ([_originAsset isKindOfClass:[PHAsset class]]){
            ...
            [manager requestAVAssetForVideo:_originAsset options:options resultHandler:^(AVAsset * _Nullable asset, AVAudioMix * _Nullable audioMix, NSDictionary * _Nullable info) {
                AVURLAsset *urlAsset = (AVURLAsset *)asset;
//                weakself.videoURL = urlAsset.URL;
                
                _cacheVideoUrl = urlAsset.URL;
                _getVideo(_cacheVideoUrl);
            }];
        }
    }
...
-(void)setGetVideo:(void (^)(NSURL *))getVideo{
    [self setGetVideo:getVideo fromNetwokProgressHandler:nil];
}
...
-(void)drawRect:(CGRect)rect{
    ...
    [self.text drawAtPoint:CGPointMake(rect.size.width - titleSize.width - 2 , (height - 12) / 2) withAttributes:@{NSFontAttributeName:self.font,NSForegroundColorAttributeName:RRCHighLightTitleColor}];

    NSBundle *r = [NSBundle bundleName:@"rrcfixbasemodule" andResourcesBundleName:@"ZYQAssetPicker"];
    UIImage *videoIcon =  [UIImage imageNamed:@"AssetsPickerVideo" inBundle:r compatibleWithTraitCollection:nil];
...
+ (void)initialize{
    NSBundle *r = [NSBundle bundleName:@"rrcfixbasemodule" andResourcesBundleName:@"ZYQAssetPicker"];
    checkedIcon =  [UIImage imageNamed:@"AssetsPickerChecked" inBundle:r compatibleWithTraitCollection:nil];
//    checkedIcon     = [UIImage imageWithContentsOfFile:[[[NSBundle bundleName:@"rrcfixbasemodule" andResourcesBundleName:@"ZYQAssetPicker"] resourcePath] stringByAppendingPathComponent:[NSString stringWithFormat:@"ZYQAssetPicker.Bundle/Images/%@@2x.png",(!IS_IOS7) ? @"AssetsPickerChecked~iOS6" : @"AssetsPickerChecked"]]];
...
//+ (NSBundle *)zyq_imagePickerBundle {
//    NSString *bundleName = @"rrcfixbasemodule";
...
//        resource_bundle = [NSBundle bundleWithURL:bundleURL];
//    }
//    NSURL *url = [resource_bundle URLForResource:@"ZYQAssetPicker" withExtension:@"bundle"];
...
-(void)addFootview{
    ...
    //UIView *view =[[UIView alloc]initWithFrame:CGRectMake(0,self.tableView.frame.size.height-95,kScreenWidth, 44)];
    self.bottomView.backgroundColor =RGB(240, 240, 240);
    ...
    [btn addTarget:self action:@selector(preViewPhoto) forControlEvents:UIControlEventTouchUpInside];
    [self.bottomView addSubview:btn];
    UIButton *sendbtn =[UIButton buttonWithType:UIButtonTypeCustom];
    [sendbtn setTitle:@"确认" forState:UIControlStateNormal];
    [sendbtn setTitleColor:RGB(14, 99, 254) forState:UIControlStateNormal];
    sendbtn.frame =CGRectMake(kScreenWidth - 60, 0,60, 44);
    [sendbtn addTarget:self action:@selector(finishPickingAssets:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)preViewPhoto{
    ...
    if (_indexPathsForSelectedItems.count <= 0) {
        ...
        for (ZYQAsset *asset in _indexPathsForSelectedItems) {
            if ([asset mediaType] == ZYQAssetMediaTypeImage) {
                if ([asset.originAsset isKindOfClass:[ALAsset class]]) {
                    ALAsset *assets = (ALAsset*)asset.originAsset;
                    UIImage *tempImg=[self normalizedImage:[UIImage imageWithCGImage:assets.defaultRepresentation.fullScreenImage]];
                    ...
                    CGFloat aspectRatio = ((PHAsset*)asset.originAsset).pixelWidth / (CGFloat)((PHAsset*)asset.originAsset).pixelHeight;
                    CGFloat multiple = [UIScreen mainScreen].scale;
                    CGFloat pixelWidth = photoWidth * multiple;
                    CGFloat pixelHeight = pixelWidth / aspectRatio;
                    ...
                    //                }
                    //            };
                    [[PHImageManager defaultManager] requestImageForAsset:(PHAsset*)asset.originAsset targetSize:CGSizeMake(pixelWidth, pixelHeight) contentMode:PHImageContentModeAspectFill options:requestOptions resultHandler:^(UIImage *result, NSDictionary *info){
                        BOOL downloadFinined = ![[info objectForKey:PHImageCancelledKey] boolValue] && ![info objectForKey:PHImageErrorKey] && ![[info objectForKey:PHImageResultIsDegradedKey] boolValue];
                        
                        //设置BOOL判断，确定返回高清照片
                        if (downloadFinined) {
                            UIImage *tempImg=[self normalizedImage:result];
                            [arr addObject:tempImg];
                        }
                    }];
        ...
        MGShowImage *ymImageV = [[MGShowImage alloc]initWithFrame:maskview.bounds byClick:0 appendArray:arr andImageArr:@[@"image"] isLocal:YES];
...
- (void)setupGroup
{
    ...
    if (IS_IOS8) {
        ...
        for (NSInteger i = 0; i < customAlbumsFetchResult.count; i++) {
            showNotAllowed=NO;
            // 获取一个相册（PHAssetCollection）
            PHCollection *collection = customAlbumsFetchResult[i];
            if ([collection isKindOfClass:[PHAssetCollection class]]) {
                PHAssetCollection *assetCollection = (PHAssetCollection *)collection;
                // 从每一个智能相册中获取到的 PHFetchResult 中包含的才是真正的资源（PHAsset）
                PHFetchOptions *fetchOptionsAlbums=[[PHFetchOptions alloc] init];

                PHFetchResult *fetchResult = [PHAsset fetchAssetsInAssetCollection:assetCollection options:fetchOptionsAlbums];
                if (fetchResult.count>0) {
                    ZYQAssetsGroup *tmpGroup=[[ZYQAssetsGroup alloc] init];
                    tmpGroup.originAssetGroup=collection;
                    tmpGroup.originFetchResult=fetchResult;
...
- (void)showNotAllowed
{
    ...
    NSBundle *r = [NSBundle bundleName:@"rrcfixbasemodule" andResourcesBundleName:@"ZYQAssetPicker"];
    UIImage *podImage =  [UIImage imageNamed:@"AssetsPickerLocked" inBundle:r compatibleWithTraitCollection:nil];
    UIImageView *padlock    = [[UIImageView alloc] initWithImage:podImage];
...
- (id)init
{
    ...
    {
        _maximumNumberOfSelection      = MAXFLOAT;
