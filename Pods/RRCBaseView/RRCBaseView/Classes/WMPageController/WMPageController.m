...
// 用于缓存加载过的控制器
@property (nonatomic, strong,readwrite) NSCache *memCache;
...
- (void)didEnterController:(UIViewController *)vc atIndex:(NSInteger)index {
    ...
    if (length == 3) {
        length = (int)self.childControllersCount - 1;
    }
    int start = 0;
...
- (void)wm_setup {
    ...
    _titleColorSelected = RRCHighLightTitleColor;
    _titleColorNormal   = RRCGrayTextColor;
    _titleColorSelectedS= RRCF21646Str;
    _titleColorNormalS  = RRCGrayTextS;
    _menuBackViewColor  = RRCUnitViewColor;
    _menuItemWidth = 65.0f;
    _progressColor = RRCProgressViewColor;
...
- (void)wm_addMenuView {
    ...
    if (self.menuBackViewColor) {
        menuView.backgroundColor = self.menuBackViewColor;
    }
...
- (CGFloat)wm_calculateItemWithAtIndex:(NSInteger)index {
    NSString *title = [self titleAtIndex:index];
    UIFont *titleFont = self.titleFontName ? [UIFont fontWithName:self.titleFontName size:self.titleSizeSelected] : [UIFont fontWithName:@"PingFangSC-Semibold" size:self.titleSizeSelected];
...
-(void)co:(CGPoint)point{
    ...
    if (_startDragging) {
        ...
        [self.scrollView setContentOffset:CGPointMake(contentOffsetX, point.y)];
    }
}

-(void)co_end{
    self.menuView.userInteractionEnabled = YES;
    //拖曳结束，如果拖曳的距离大于一半时，就滚动过去
    if (self.scrollView.contentOffset.x > _contentViewFrame.size.width/2) {
        [self.scrollView setContentOffset:CGPointMake(_contentViewFrame.size.width, 0) animated:YES];
    }else{
        [self.scrollView setContentOffset:CGPointMake(0 , 0) animated:YES];
    }
    
    _selectIndex = (int)(self.scrollView.contentOffset.x / _contentViewFrame.size.width);
    self.currentViewController = self.displayVC[@(self.selectIndex)];
    [self didEnterController:self.currentViewController atIndex:self.selectIndex];
...
-(BOOL)menuView:(WMMenuView *)menu shouldSelesctedIndex:(NSInteger)index{
    if (self.delegate && [self.delegate respondsToSelector:@selector(pageController:shouldSelesctedIndex:withInfo:)]) {
        return  [self.delegate pageController:self shouldSelesctedIndex:index withInfo:[self infoWithIndex:index]];
    }
    return YES;
}
...
-(NSString *)menuView:(WMMenuView *)menu titleColorSForState:(WMMenuItemState)state atIndex:(NSInteger)index{
    switch (state) {
        case WMMenuItemStateSelected: return self.titleColorSelectedS;
        case WMMenuItemStateNormal: return self.titleColorNormalS;
    }
}
...
- (UIView *)menuView:(WMMenuView *)menu badgeViewAtIndex:(NSInteger)index{
    if (self.dataSource && [self.dataSource respondsToSelector:@selector(pageController:badgeViewAtIndex:)]) {
        return [self.dataSource pageController:menu badgeViewAtIndex:index];
    }else{
        return nil;
    }
}
