...
- (NSString *)colorSForState:(WMMenuItemState)state atIndex:(NSInteger)index {
    if ([self.delegate respondsToSelector:@selector(menuView:titleColorForState:atIndex:)]) {
        return [self.delegate menuView:self titleColorSForState:state atIndex:index];
    }
    return RRCGrayTextS;
}
...
- (void)slideMenuAtProgress:(CGFloat)progress {
    ...
    if (rate == 0.0) {
        ...
        return;
    }else if (rate >= 0.5){//当滑动大于一半时，改变字体颜色 0->1
        currentItem.rate = 0.0;
        nextItem.rate = 1.0;
    }else{
        currentItem.rate = 1.0;
        nextItem.rate = 0.0;
    }
    
    [currentItem setSelected:currentItem.rate == 1.0 ? YES : NO withAnimation:NO];
    [nextItem setSelected:nextItem.rate == 1.0 ? YES : NO withAnimation:NO];
    
}

- (void)selectItemAtIndex:(NSInteger)index {
    [self selectItemAtIndex:index animations:NO];
}
- (void)selectItemAtIndex:(NSInteger)index animations:(BOOL)animations{
...
- (void)resetFrames {
    ...
    self.menuLineView.frame = CGRectMake(0, frame.size.height - 0.5, self.frame.size.width, 0.5);
    [self resetFramesFromIndex:0];
}
...
- (void)resetFramesFromIndex:(NSInteger)index {
    ...
    if (self.style == WMMenuViewStyleLine || self.style == WMMenuViewStyleTriangle) {
        self.progressHeight = self.progressHeight > 0 ? self.progressHeight : 3.0;
...
- (void)addScrollView {
    ...
    self.menuLineView.frame = CGRectMake(0, height - 1, width, 1);
    
}

-(UIView *)menuLineView{
    if (!_menuLineView) {
        _menuLineView = [UIView new];
        _menuLineView.backgroundColor = RRCLineViewColor;
    }
    return _menuLineView;
}
...
- (void)addItems {
    ...
    for (int i = 0; i < self.titlesCount; i++) {
        ...
        item.normalColorS  = [self colorSForState:WMMenuItemStateNormal atIndex:i];
        item.selectedColorS= [self colorSForState:WMMenuItemStateSelected atIndex:i];
