...
typedef NS_ENUM(NSUInteger, WMPageControllerPreloadPolicy) {
    ...
    WMPageControllerPreloadPolicyAll      = 3  // Pre-load all controllers near the current.
...
//定制菜单
- (UIView *)pageController:(WMMenuView *)menu badgeViewAtIndex:(NSInteger)index;
@end
...
- (BOOL)pageController:(WMPageController *)pageController shouldSelesctedIndex:(NSInteger)index withInfo:(NSDictionary *)info;
/**
 ...
 */
@property (nonatomic, strong) NSString *titleColorSelectedS;
 ...
 */
@property (nonatomic, strong) NSString *titleColorNormalS;
...
/// 菜单背景视图
@property (nonatomic, strong) UIColor *menuBackViewColor;
/** 内部容器 */
