...
-(void)layoutSubviews{
    ...
    //更新选中字体颜色
    UIColor *selectedTextColor = self.selected ? RRCTEXTCOLOR(self.selectedColorS) : RRCTEXTCOLOR(self.normalColorS);
    self.textColor = selectedTextColor;
    
    self.selectedColor = RRCTEXTCOLOR(self.selectedColorS);
    self.normalColor   = RRCTEXTCOLOR(self.normalColorS);
}

#pragma mark - Public Methods
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        self.normalColorS  = RRCGrayTextS;
        self.selectedColorS= RRCF21646Str;
        self.normalColor   = RRCGrayTextColor;
        self.selectedColor = RRCHighLightTitleColor;
        self.normalSize    = 15;
...
- (void)setSelected:(BOOL)selected withAnimation:(BOOL)animation {
    ...
    [UIView animateWithDuration:0.5f animations:^{
        if (selected) {
            self.font = [UIFont fontWithName:@"PingFangSC-Semibold" size:self.normalSize];;
        }
        else {
            self.font = [UIFont fontWithName:@"PingFangSC-Regular" size:self.normalSize];
        }
    }];
