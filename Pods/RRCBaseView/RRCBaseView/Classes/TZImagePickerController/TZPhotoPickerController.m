...
@property (nonatomic,strong) TZNavTop *navTop;
...
-(void)dropViewAction:(BOOL)isUnfold{
    if (isUnfold) {
        [self.view addSubview:self.dropView];
        [self.dropView addSubview:self.tableView];
        [self configTableView];
    }
    
    [UIView animateWithDuration:.3 animations:^{
        if (isUnfold) {
            self.tableView.tz_height = kScreenHeight - 120*Device_Ccale - iPhoneXBottomUnSafeAreaHeight;
        }else{
            self.tableView.tz_height = 0;
        }
...
-(UIView *)dropView{
    if (!_dropView) {
        _dropView = [[UIView alloc] initWithFrame:CGRectMake(0, kHeightNavigation, kScreenWidth, kScreenHeight)];
...
-(void)configTableView{
    [[TZImageManager manager] getAllAlbums:NO allowPickingImage:YES needFetchAssets:!self.isFirstAppear completion:^(NSArray<TZAlbumModel *> *models) {
...
//                [self configTableView];
            ...
            if (!self->_tableView) {
//                self->_tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight - 80 - iPhoneXBottomUnSafeAreaHeight) style:UITableViewStylePlain];
...
//                [self.dropView addSubview:self->_tableView];
            } else {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [_navTop dropViewAction];
    self.columnNumber = self.columnNumber;
    TZAlbumModel *model = _albumArr[indexPath.row];
    self.model = model;
    self.navTop.navStr = model.name;
    self.navTop.tz_left = (kScreenWidth - self.navTop.tz_width)/2. + 12;
    [self.dropView removeFromSuperview];
...
-(TZNavTop *)navTop{
    if (!_navTop) {
        _navTop = [[TZNavTop alloc] initWithFrame:CGRectMake(0, 0, 0, 44)];
        kWeakSelf;
        _navTop.DropBlock = ^(BOOL isProp) {
            [weakSelf dropViewAction:isProp];
        };
    }
    return _navTop;
}
...
- (void)viewDidLoad {
    ...
    self.view.backgroundColor = RRCUnitViewColor;
    
    [self.navigationController.navigationBar addSubview:self.navTop];
//    self.navTop.width = 100;
//    self.navTop.centerX = self.navigationController.navigationBar.centerX;
//    [self.navTop mas_makeConstraints:^(MASConstraintMaker *make) {
//        make.height.mas_offset(44);
//        make.centerX.mas_equalTo(self.navigationController.navigationBar);
//        make.centerY.mas_equalTo(self.navigationController.navigationBar);
//    }];
    
    self.navTop.navStr = _model.name;
    self.navTop.tz_left = (kScreenWidth - self.navTop.tz_width)/2. + 12;
...
- (void)configCollectionView {
    ...
    _collectionView.backgroundColor = RRCUnitViewColor;
...
- (void)configBottomToolBar {
    ...
    _bottomToolBar.backgroundColor = RRCUnitViewColor;
    ...
    [_previewButton setTitleColor:RRCThemeTextColor forState:UIControlStateNormal];
    [_previewButton setTitleColor:RRCGrayTextColor forState:UIControlStateDisabled];
    ...
    _divideLine.backgroundColor = RRCLineViewColor;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tzImagePickerVc.sortAscendingByModificationDate || !_showTakePhotoBtn) {
        if (_model.count > indexPath.item) {
