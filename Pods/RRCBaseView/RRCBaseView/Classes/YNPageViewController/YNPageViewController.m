...
NSString *const YNPageViewControllerHeadViewResetNotification = @"YNPageViewControllerHeadViewResetNotification";
...
- (void)viewDidLoad {
    ...
    //循环加入缓存
    if (self.config.preloadPolicy != YNPageViewControllerPreloadPolicyNever){
        NSInteger length = self.config.preloadPolicy;
        if (self.config.preloadPolicy == YNPageViewControllerPreloadPolicyAll) {
            length = (int)self.controllersM.count - 1;
        }
        NSInteger start = self.pageIndex;
        NSInteger end = self.controllersM.count - 1;
        
        if (self.pageIndex > length) {
            start = length;
        }
        //
        if (self.controllersM.count - 1 > length + self.pageIndex) {
            end = start + length;
        }
        for (NSInteger i = start; i <= end; i++) {
            // 如果已存在，不需要预加载
            NSString *p_title = [self titleWithIndex:i];
            if (![self.cacheDictM objectForKey:[self getKeyWithTitle:p_title]] && ![self.displayDictM objectForKey:[self getKeyWithTitle:p_title]]) {
                [self initViewControllerWithIndex:i];
            }
        }
    }
...
- (void)addViewControllerToParent:(UIViewController *)viewController index:(NSInteger)index {
    ...
    if ([self isSuspensionBottomStyle] || [self isSuspensionTopStyle]) {
        ...
        if (self.cacheDictM.count == 0) {
            ...
            CGFloat scrollMenuViewY = [self.scrollMenuView.superview convertRect:self.scrollMenuView.frame toView:self.view].origin.y - navHeight;
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    ...
    }else if ([self isSuspensionBottomStyle] || [self isSuspensionTopStyle] || [self isTopStyle]){
        if (self.delegate && [self.delegate respondsToSelector:@selector(pageViewController:didBeginDringting:)]) {
            [self.delegate pageViewController:self didBeginDringting:scrollView];
        }
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    [self removeViewController];//暂时注释
...
- (void)yn_pageScrollViewDidScrollView:(UIScrollView *)scrollView {
    ...
    if ([self isSuspensionBottomStyle] || [self isSuspensionTopStyle]) {
        ...
        if (offsetY > - self.scrollMenuView.yn_height - self.config.suspenOffsetY) {
            ...
            [self adjustSectionTopHidden:NO];
        } else {
            /// headerView往下拉置顶
            ...
            [self adjustSectionTopHidden:YES];
        }
...
//调整菜单
-(void)adjustSectionTopHidden:(BOOL)isHidden{
    if (self.config.showTopLine) {
        self.scrollMenuView.topLine.hidden = isHidden;
    }
}
...
-(void)reloadSuspendHeaderLastTopHeight:(CGFloat)lastTopHeight{
    if (self.headerView && ([self isSuspensionTopStyle] || [self isSuspensionBottomStyle])) {
        /// 重新初始化headerBgView
        [self setupHeaderBgView];
        
        CGPoint lastPointOffset = self.currentScrollView.contentOffset;
        ...
        [self.currentScrollView setContentOffset:CGPointMake(0, -self.config.tempTopHeight + (lastPointOffset.y + lastTopHeight)) animated:NO];
...
-(void)reloadSuspendHeaderLastTopHeight2:(CGFloat)lastTopHeight{
     if (self.headerView && ([self isSuspensionTopStyle] || [self isSuspensionBottomStyle])) {
           /// 重新初始化headerBgView
         [self setupHeaderBgView];
...
- (void)setupPageScrollView {
    ...
    if ([self isSuspensionTopPauseStyle]) {
        ...
        self.config.remainHeight = self.config.contentHeight - self.config.tempTopHeight;
        ...
        self.pageScrollView.frame = CGRectMake(0, [self isTopStyle] ? self.config.menuHeight : navHeight, kYNPAGE_SCREEN_WIDTH, ([self isTopStyle] ? contentHeight - self.config.menuHeight : contentHeight));
    ...
    //headView高度有值 或者更新执行通知操作
    [self initHeadBgViewAfter];
}
...
- (void)setupHeaderBgView {
    if ([self isSuspensionBottomStyle] || [self isSuspensionTopStyle] || [self isSuspensionTopPauseStyle]) {
        ...
        self.headerBgView.failScolleGes = self.config.headerViewCouldFailScrollPage;
        self.headerBgView.panHeight     = self.headerView.yn_height;
...
/// 初始完毕headView之后的事情
-(void)initHeadBgViewAfter{
    [[NSNotificationCenter defaultCenter]postNotificationName:YNPageViewControllerHeadViewResetNotification object:self.config];
}
...
//#warning 在社区关注页面刷新h滑到广场页面,整个屏幕会向下动一下.
// 将headerView 从 tableview 上 放置 view 上
- (void)replaceHeaderViewFromTableView {
    
    if ([self isSuspensionBottomStyle] || [self isSuspensionTopStyle]) {
        if (_headerViewInTableView) {
            CGFloat height = self.config.showNavigation ? kYNPAGE_NAVHEIGHT:0;
            CGFloat headerViewY = [self.headerBgView.superview convertRect:self.headerBgView.frame toView:self.pageScrollView].origin.y + height;
            CGFloat scrollMenuViewY = [self.scrollMenuView.superview convertRect:self.scrollMenuView.frame toView:self.pageScrollView].origin.y + height;
...
- (YNPageScrollView *)pageScrollView {
    if (!_pageScrollView) {
        ...
        _pageScrollView.panHeight     = self.headerBgView.yn_height;
        _pageScrollView.failScolleGes = self.config.headerViewCouldFailScrollPage;
        _pageScrollView.pagingEnabled = YES;
        _pageScrollView.bounces = NO;
        _pageScrollView.delegate = self;
        _pageScrollView.tag = 100;
